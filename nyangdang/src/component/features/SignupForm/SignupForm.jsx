...
function SignupForm() {
  ...
  const onIdCheckHandler = async (username) => {
    ...
    const response = await axios.get(
      `${process.env.REACT_APP_URL}/api/user/${username}/exists`,
      {
  ...
  const submitHandler = async (e) => {
    ...
    await axios
      .post(`${process.env.REACT_APP_URL}/api/user/signup`, body, {
        withCredentials: true,
      })
      .then((res) => {
    ...
    <StContainer>
      ...
      <StContainerForm onSubmit={submitHandler}>
        <StIdBox>
          ...
          <StButtonCheck type="button" onClick={() => onIdCheckHandler(userId)}>
            {" "}
            중복확인
